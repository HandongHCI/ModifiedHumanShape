...
function [vertex,confidence,face] = read_ply(filename)
...
if (isfield(d.vertex, 'confidence'))
    confidence = [d.vertex.confidence];
else
    confidence = ones(size(d.vertex.x));
end

% if (isfield(d.vertex, 'nx')&&isfield(d.vertex, 'ny')&&isfield(d.vertex, 'nz'))
%     normals = [d.vertex.nx d.vertex.ny d.vertex.nz];
% else
%     normals = zeros(0,3);
% end
